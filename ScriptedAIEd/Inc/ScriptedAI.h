...
struct F2DBox
{
	...
	F2DBox()
	{}
	F2DBox(const FVector2D& V)
		: Min(V), Max(V)
	{}
	F2DBox(const FVector2D& MinV, const FVector2D& MaxV)
		: Min(MinV), Max(MaxV)
	{}
	...
	inline UBOOL BoundsOverlap(const F2DBox& B) const
	{
#define CHECK_AXIS(axis) ((Min.axis >= B.Min.axis) && (Min.axis <= B.Max.axis) || (B.Min.axis >= Min.axis) && (B.Min.axis <= Max.axis))
		return CHECK_AXIS(X) && CHECK_AXIS(Y);
#undef CHECK_AXIS
	}
	inline UBOOL PointInside(const FVector2D& V) const
	{
