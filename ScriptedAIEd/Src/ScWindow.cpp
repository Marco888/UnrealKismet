...
public:
	...
	TArray<FString> KismetObjectItems;
	...
	void SelectKismet(const TCHAR* Name);
	void DeleteKismet(const TCHAR* Name);
...
public:
	...
	WNewKismetWindow(const TCHAR* WinTitle, const TCHAR* Question, const TCHAR* DefString)
		: WCustomWindowBase(0)
		, Prompt(this)
		...
		, EditField(this)
	{
		PerformCreateWindowEx(0, TEXT("DummyWindow"), WS_SYSMENU, 150, 150, 250, 150, BrowserWindow->hWnd, NULL, NULL);
		SetText(WinTitle);
		...
		Prompt.MoveWindow(10, 20, XSize - 20, 16, 1);

		EditField.OpenWindow(1, 0, 0);
		EditField.MoveWindow(25, 42, XSize - 50, 20, 1);
		EditField.SetText(DefString);

		OkButton.OpenWindow(1, 80, YSize - 40, 50, 20, TEXT("OK"));
		CancelButton.OpenWindow(1, 145, YSize - 40, 50, 20, TEXT("Cancel"));
	}
};
...
public:
	// Constructor.
	WNewKismetDialog()
		: WNewKismetWindow(TEXT("New ScriptedAI trigger object"), TEXT("Name of object:"), GrabUniqueName())
	{
	}
	...
	void OnClickTrue()
	{
		...
		if (bResult)
		{
			AScriptedAI* MG = (AScriptedAI*)Level->SpawnActor(AScriptedAI::StaticClass(), NewObjName);
			if (MG)
			{
				BrowserWindow->SelectKismet(*NewObjName);
...
class WNewGroupDialog : public WNewKismetWindow
{
public:
	INT XPos, YPos;
	...
	WNewGroupDialog(INT X, INT Y)
		: WNewKismetWindow(TEXT("New group..."), TEXT("Group description:"), TEXT(""))
		, XPos(X), YPos(Y)
	{}

	void OnClickTrue()
	{
		BrowserWindow->API->AddNewGroup(EditField.GetText(), XPos, YPos);
		WNewKismetWindow::OnClickTrue();
	}
};
class WEditGroupDialog : public WNewKismetWindow
{
public:
	INT GroupIndex;

	// Constructor.
	WEditGroupDialog(const TCHAR* OldText, INT GIndex)
		: WNewKismetWindow(TEXT("Edit group..."), TEXT("Group description:"), OldText)
		, GroupIndex(GIndex)
	{}

	void OnClickTrue()
	{
		BrowserWindow->API->EditGroupText(EditField.GetText(), GroupIndex);
		WNewKismetWindow::OnClickTrue();
	}
};
...
void WKismetBrowserWindow::UpdateSelection()
{
	...
	for(TActorIterator<AScriptedAI> It(GetLevel()); It; ++It)
	{
		...
		if (M) // Add info.
		{
			if (!M->bInit)
				M->Initialize();
			const TCHAR* Name = M->GetName();
			AppendMenuW(hKismetObjectsMenu, MF_STRING | (M == SelectedManager ? MF_CHECKED : 0), ID_KISMETITEM + KismetObjectItems.Num(), Name);
			AppendMenuW(hKismetObjectDel, MF_STRING | (M == SelectedManager ? MF_CHECKED : 0), ID_DELKISMETITEM + KismetObjectItems.Num(), Name);
			new (KismetObjectItems) FString(Name);
		}
	}
	UpdateWindow(hWnd);
	pViewport->RepaintPending = TRUE;
...
void WKismetBrowserWindow::SelectKismet(const TCHAR* Name)
{
	guard(WKismetBrowserWindow::SelectKismet);
	SelectedManager = FindObject<AScriptedAI>(ANY_PACKAGE, Name, 0);
	if (SelectedManager)
		API->SetManager(SelectedManager);
	unguard;
}
void WKismetBrowserWindow::DeleteKismet(const TCHAR* Name)
{
	guard(WKismetBrowserWindow::DeleteKismet);
	AScriptedAI* M = FindObject<AScriptedAI>(ANY_PACKAGE, Name, 0);
	if (!M || M->bDeleteMe)
		return;
...
void FGroupObject::Draw(FSceneNode* Frame, const FRenderArea& Area)
{
	...
	UCanvas* C = Frame->Viewport->Canvas;
	C->DrawColor = FColor(128, 250, 255, 255);
	C->CurX = Pos.X;
	C->CurY = Area.ToScreenY(TextY);
	C->WrappedPrintf(UWhiteFont, FALSE, TEXT("%ls"), *Title);

	const FPlane EdgeColor(Highlight ? FPlane(0.89f, 0.89f, 0.65f, 1.f) : FPlane(0.54f, 0.54f, 0.54f, 1.f));
	...
	Frame->Viewport->RenDev->DrawTile(Frame, *GroupTexInfo, Pos.X + EdgePixels, Pos.Y + EdgePixels, Scale.X - (EdgePixels * 2.f), Scale.Y - (EdgePixels * 2.f), 0.f, 0.f, 1.f, 1.f, nullptr, 1.f, FPlane(1.f, 1.f, 1.f, 1.f), FPlane(0, 0, 0, 0), PF_Modulated);
	unguard;
}
...
void FKismetAPI::DrawKismet(FSceneNode* Frame)
{
	...
	GroupTexInfo = GroupTex->GetTexture(INDEX_NONE, Frame->Viewport->RenDev);
...
void FKismetAPI::TraceMouse(FLOAT X, FLOAT Y)
{
	...
	for (FRenderEntry* RE = RenderList; RE; RE = RE->GetNext())
		if (RE->Bounds.PointInside(Mouse))
...
public:
	...
	{
		guard(FClassesHandler::FClassesHandler);
		for (TObjectIterator<UClass> It; It; ++It)
		{
			UClass* C = *It;
			if (!(C->ClassFlags & CLASS_Abstract))
			{
				if (C->IsChildOf(USObjectBase::StaticClass()))
					AddAction(C);
			}
		}
	...
	void ListActionClasses(FRightClickMenu& Menu)
	{
...
void FKismetAPI::Click(DWORD Buttons, FLOAT X, FLOAT Y)
{
	...
	else if (Buttons == MOUSE_Right)
	{
		...
		// Bring dropdown menu.
		{
			...
			{
				ClassHandle.ListActionClasses(SelectMenu);
				SelectMenu.AddItem();
		...
		case ID_EDITGROUP:
			{
				...
				WEditGroupDialog* G = new WEditGroupDialog(*Manager->GroupsInfo(ix).GroupInfo, ix);
				G->Show(1);
			}
			break;
		case ID_DELSELACTOR:
...
FObjectEntry* FKismetAPI::AddAction(const FVector2D& Position, UClass* UC)
{
	...
	USObjectBase* KObj = ConstructObject<USObjectBase>(UC, Manager->GetOuter());
	if (!KObj)
	{
		BrowserWindow->Logf(TEXT("Couldn't add action %ls for an unknown reason."), UC->GetFullName());
		return nullptr;
	}
	Manager->Actions.AddItem(KObj);

	KObj->Location[0] = appRound(Position.X);
	KObj->Location[1] = appRound(Position.Y);
	KObj->Level = Manager->Level;
	KObj->Trigger = Manager;
	Manager->CheckStatic();

	if (KObj->bRequestBeginPlay)
	{
		KObj->NextBeginPlay = Manager->BeginPlayList;
		Manager->BeginPlayList = KObj;
	}
